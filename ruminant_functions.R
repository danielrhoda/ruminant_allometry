...
# an old function I would use to calculate grids along a morphospace, there's a quicker way to do this though (expand.grid() ... )
  ...
  # a rough draft of the "tps" function
...
# calculates centroid size for 1 landmark configuration
centroid1 <- function(lm){apply(lm, 2, mean)}
...
# modified function from the web
# adds transparency to a or a vector of color codes
  addTrans <- function(color,trans){
  trans <- trans*255
  if (length(color)!=length(trans)&!any(c(length(color),length(trans))==1)) stop("Vector lengths not correct")
  if (length(color)==1 & length(trans)>1) color <- rep(color,length(trans))
...
#########
  ...
  b<-seq(ym-pad.y, yM+pad.y,length=m) #by=(xM-xm)*7/(5*(n-1))) # y values of grid points
  M<-as.matrix(expand.grid(a,b))
  ngrid<-tps2d(M,matr,matt)

  grid.color <- addTrans(grid.color, grid.transparency)
  lm.color <- addTrans(lm.color, lm.transparency)
  link.color <- addTrans(link.color, link.transparency)

# if one of the shade.method or palette parameters aren't NULL, we're plotting with color:
...
# warped array of cells
  # storing the gridpoints that define each cell in an array
for(i in 1:nrow){
rows <- array(dim=c(4,2,ncol))
  for(j in 1:ncol){
  rows[1,,j] <- ngrid[1+(j-1)+((i-1)*n),]
  rows[2,,j] <- ngrid[1+(j-1)+n+((i-1)*n),]
  rows[3,,j] <- ngrid[1+j+n+((i-1)*n),]
  rows[4,,j] <- ngrid[1+j+((i-1)*n),]
  }
if(i==1){cell.array<-rows}else{
cell.array <- bindArr(cell.array,rows,along=3)}
}
...
rows <- array(dim=c(4,2,ncol))
  for(j in 1:ncol){
  rows[1,,j] <- M[1+(j-1)+((i-1)*n),]
  rows[2,,j] <- M[1+(j-1)+n+((i-1)*n),]
  rows[3,,j] <- M[1+j+n+((i-1)*n),]
  rows[4,,j] <- M[1+j+((i-1)*n),]
  }
if(i==1){cell.array.ref<-rows}else{
cell.array.ref <- bindArr(cell.array.ref,rows,along=3)}
}
...
grid.id <- expand.grid(1:nc,1:nr) # making row and columns id's of each grid cell
...
cell.array <- cell.array[,,-cells.to.drop]
ncell <- ncell-length(cells.to.drop)
...
if(is.null(palette) && length(shade.method) == 3){
plot(ngrid, cex=0.01,asp=1,axes=F,xlab="",ylab="",col='transparent')
for (i in 1:m){lines(ngrid[(1:n)+(i-1)*n,],lwd=grid.width,col=grid.color)}
for (i in 1:n){lines(ngrid[(1:m)*n-i+1,],lwd=grid.width,col=grid.color)}
if(plot.lm){points(M2,pch = 19,cex=lm.size, col = lm.color)}
if(!is.null(links)){
for (i in 1:nrow(links)) {
segments(M2[links[i, 1], 1], M2[links[i, 1], 2], M2[links[i, 2], 1], M2[links[i, 2], 2],lwd = link.size, col = link.color)}
}}

require(colorBlindness)

  if(is.null(palette) && length(shade.method) != 3){
    if(shade.method == "shape.proc" | shade.method == "shape.natural"){
      palette <- colorRampPalette(LightBlue2DarkBlue10Steps)(ncell)}else{
      palette <- colorRampPalette(Blue2DarkRed18Steps)(ncell)}}
  if(!is.null(palette) && length(shade.method) == 3){  # area is the default way I'm coloring the cells
    shade.method <- 'area'}

if(shade.method=='area'){
areas <- c()
areas.ref <- c()
diff <- c()
for(i in 1:ncell){
  area.target <- polyarea(cell.array[,1,i],cell.array[,2,i])
  area.reference <- polyarea(cell.array.ref[,1,i],cell.array.ref[,2,i])
  diff[[i]] <- area.target-area.reference
  }
diff <- as.vector(unlist(diff))
}

if(shade.method=='shape.proc'){
require(shapes)
proc.dists <- c()
normal.cell <- cell.array.ref[,,1]
all.cells <- bindArr(normal.cell,cell.array,along=3)
cell.proc <- gpagen(all.cells,print.progress = FALSE)$coords
for(i in 1:dim(cell.array)[[3]]){
  proc.dists[[i]] <- procdist(cell.proc[,,1],cell.proc[,,i+1])
}
proc.dists <- unlist(proc.dists)
diff <- proc.dists
}

if(shade.method=='shape.natural'){
require(shapes)
trans1 <- function(M) {M-matrix(centroid1(M),nrow(M),ncol(M),byrow=T)}
proc.dists <- c()
normal.cell <- cell.array.ref[,,1]
all.cells <- bindArr(normal.cell,cell.array,along=3)
all.cells.nat <- all.cells
for(i in 1:dim(cell.array)[[3]]){
  all.cells.nat[,,i+1] <- trans1(cell.array[,,i]) # doesn't rotate or re-scale the cells
}
cell.proc <- all.cells.nat
for(i in 1:dim(cell.array)[[3]]){
  proc.dists[[i]] <- procdist(cell.proc[,,1],cell.proc[,,i+1])
}
proc.dists <- unlist(proc.dists)
...
# plotting the TPS with shade
if(abs){diff <- abs(rescale.numeric(diff,to=c(-1,1)))}
if(!is.null(shade.scale)){diff <- as.vector(unlist(c(cs1[1],diff,cs1[2])))}
dit2<-cut(as.numeric(diff),breaks = 100)
palette <- addTrans(color = palette, trans = shade.transparency)
idcolor <- palette

plot(ngrid, cex=0.01,asp=1,axes=F,xlab="",ylab="",col='transparent')
for(i in 1:ncell){
  if(!is.null(shade.scale)){j <- i+1} else{j <- i}
  polygon(x = cell.array[,,i], col = idcolor[as.numeric(dit2[j])], border = NA)
}
for (i in 1:m){lines(ngrid[(1:n)+(i-1)*n,],lwd=grid.width,col=grid.color)}
for (i in 1:n){lines(ngrid[(1:m)*n-i+1,],lwd=grid.width,col=grid.color)}
if(plot.lm){points(M2,pch = 19,cex=lm.size, col = lm.color)}
